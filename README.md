...
4. An implementation of Cedric Scheerlinck [paper](https://arxiv.org/abs/1811.00386/) to reconstruct images from events asynchronously, visualised in the GIF above. My package can be found in `evis_ws/src/dvs_integrator`.
...
6. A ROS node for computing a global optical flow vector for the image, by contrast maximisation, based on Prof. Gallego's [paper](https://rpg.ifi.uzh.ch/docs/CVPR18_Gallego.pdf). This node is implemented in Python to allow me to prototype faster, but a C++ node maybe developed later on. Note while building the workspace to **NOT** use the `--symlink-install` option as it fails and breaks the workspace due to the `ament_cmake_python` build tool. Use the `clean_build.bash` script in case your build breaks. My package can be found in `evis_ws/src/dvs_global_flow`.
7. A ROS node for reconstructing absolute intensity images asynchronously from events and camera orientation. First, an edge and gradient map is computed from the events, then it is integrated using Poisson to recover the image. My package can be found in `evis_ws/src/dvs_mosaic`.
